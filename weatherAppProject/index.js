...
async function fetchUserWeatherInfo(coordinates) {
    ...
    try {
        const response = await fetch(
          ...
          // "jsonformatter.curiousconcept" pe detail milega.
...
function renderWeatherInfo(weatherInfo) {
    ...
    const windDeg = document.querySelector("[data-windDeg]");
    ...
    pressure.innerText = `${weatherInfo?.main?.pressure} Pa`;
    sunrise.innerText = `${weatherInfo?.sys?.sunrise}`;
    sunset.innerText = `${weatherInfo?.sys?.sunset}`;
    windDeg.innerText = `${weatherInfo?.wind?.deg}  °C`;
